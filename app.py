...
with col3:
    ...
    bmi = st.slider("Body Mass Index (BMI)", 12.0, 50.0, 20.0, format="%.1f")
...
with col4:
    ...
    annual_medical_cost = st.number_input("Last Year’s Medical Expenses ($)", min_value=0.0, value=1000.0, format="%.2f")
...
with col16:
    ...
    hypertension = st.selectbox("Hypertension", options=[0, 1], format_func=lambda x: "Yes" if x == 1 else "No", help="Please refer to the Blood Pressure Category before selecting.")
    diabetes = st.selectbox("Diabetes", options=[0, 1], format_func=lambda x: "Yes" if x == 1 else "No", help="Please refer to the HbA1c Category before selecting.")
...
with col18:
    ...
    policy_term_years = st.slider("Policy Term (Years)", min_value=1, max_value=10, value=4)
