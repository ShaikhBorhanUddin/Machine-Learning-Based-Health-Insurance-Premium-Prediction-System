...
with col3:
    ...
    smoker = st.slider("Smoking Habit", options=['Never', 'Former', 'Current'])
    alcohol_freq = st.slider("Alcohol Consumption", options=['Never', 'Weekly', 'Daily', 'Occasional'])
    systolic_bp = st.slider("Systolic BP", min_value=60.0, value=240.0, format="%.1f")
    diastolic_bp = st.slider("Diastolic BP", min_value=40.0, value=140.0, format="%.1f")
with col4:
